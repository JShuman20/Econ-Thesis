...
DAT_CLEAN = DAT %>%
  ...
  mutate(EVENT_TYPE = toupper(EVENT_TYPE),
         ...
         MID_DATE = BEGIN_DATE + floor(as.numeric(END_DATE - BEGIN_DATE)/2))
#Filtering to US States Only
DAT_CLEAN = DAT_CLEAN %>%
  left_join(STATE_REGION, by = "STATE") %>%
  filter(!is.na(Region))

#Adding Variables For Placebo Group
...
DAT_CLEAN = DAT_CLEAN %>%
  select(c(1:16,18,20:26,28,30,33,44:58)) %>%
  mutate(PLACEBO_EVENT = ifelse(EVENT_TYPE %in% PLACEBO_EVENTS, 1,0))
  

#Writing Intermediate - 2 File
